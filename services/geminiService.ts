 ...
 */
export const generateBackgroundRemovedImage = async (
    originalImage: File,
): Promise<string> => {
    ...
    const originalImagePart = await fileToPart(originalImage);
    const prompt = `You are an expert photo editor AI. Your task is to accurately remove the background from the provided image.
    
Instructions:
1. Identify the main subject(s) in the image.
2. Create a precise mask around the subject(s), preserving all details like hair and fine edges.
3. Make the background completely transparent.
4. Return the resulting image as a PNG with an alpha channel.

Output: Return ONLY the final image. Do not return any text or explanation.`;
    const textPart = { text: prompt };
