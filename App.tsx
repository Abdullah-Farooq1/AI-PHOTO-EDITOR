...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateBackgroundRemovedImage } from './services/geminiService';
...
type Tab = 'retouch' | 'adjust' | 'filters' | 'crop' | 'magic';
...
const App: React.FC = () => {
  ...
  const handleRemoveBackground = useCallback(async () => {
    ...
    try {
        const resultUrl = await generateBackgroundRemovedImage(currentImage);
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'crop', 'adjust', 'filters', 'magic'] as Tab[]).map(tab => (
                 <button
                    key={tab}
        ...
        <div className="w-full">
            ...
            {activeTab === 'magic' && <MagicPanel onRemoveBackground={handleRemoveBackground} isLoading={isLoading} />}
        </div>
